...
def render_eeg_review(data_manager):
    ...
    first_reviewer = mert2_user.get(
        analysis_meta.get("reviewerStaffId"), "N/A")
    second_reviewer = mert2_user.get(
        analysis_meta.get("secondReviewerStaffId"), "N/A")
    ...
    with ui.card(key="eeg_review"):
        # Header Section
        ui.element("h3", children=[
                   "EEG Review Status"], className="text-xl font-bold mb-4", key="header_title")
        ...
        ui.element("h4", children=[
                   "First Review"], className="text-lg font-semibold mb-2", key="first_review_title")
        ui.element("span", children=[
                   "Review Date:"], className="text-gray-500 text-sm font-medium", key="first_review_date_label")
        ui.element("div", children=[format_datetime(analysis_meta.get(
            'reviewDatetime'))], className="mb-2", key="first_review_date")
        ui.element("span", children=[
                   "Reviewer:"], className="text-gray-500 text-sm font-medium", key="first_reviewer_label")
        ui.element("div", children=[first_reviewer],
                   className="mb-4", key="first_reviewer_value")
        ...
        ui.element("h4", children=[
                   "Second Review"], className="text-lg font-semibold mb-2", key="second_review_title")
        ui.element("span", children=[
                   "Review Date:"], className="text-gray-500 text-sm font-medium", key="second_review_date_label")
        ui.element("div", children=[format_datetime(analysis_meta.get(
            'secondReviewDatetime'))], className="mb-2", key="second_review_date")
        ui.element("span", children=[
                   "Reviewer:"], className="text-gray-500 text-sm font-medium", key="second_reviewer_label")
        ui.element("div", children=[second_reviewer],
                   className="mb-4", key="second_reviewer_value")
    ...
    if (can_review and current_state != EEGReviewState.REJECTED and current_state != EEGReviewState.CLINIC_REVIEW and EEGReviewState.COMPLETED):
        ...
        with col1:
            if st.button("Approve Review", type="primary", key="approve_btn"):
                # first review done by Stephanie
        ...
        with col2:
            with st.form(key="reject_form", clear_on_submit=True):
                ...
                if st.form_submit_button("Reject Review"):
                    handle_reject(data_manager, current_state,
                                  selected_reasons)
